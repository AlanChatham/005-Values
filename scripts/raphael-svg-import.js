...
Raphael.fn.importSVG = function (svgXML) {
  ...
  try {
    this.parseElement = function(elShape) {
      ...
      var attr = {"stroke": "transparent", "fill":"#000"}, i;
      if (elShape.attributes){
        for (i = elShape.attributes.length - 1; i >= 0; --i){
          // Special case for importing Inkscape 'style' attributes
          if (elShape.attributes[i].name == 'style'){
            // First, split up pairs by semicolons
            var stylePairArray = elShape.attributes[i].value.split(";");
            for (var j = 0; j < stylePairArray.length; j++){
              var stylePair = stylePairArray[j].split(":");
              //console.log(stylePair[0] +" , " + stylePair[1]);
              attr[stylePair[0]] = stylePair[1];
            }
          }
          else{
            //console.log("Found attribute: " + elShape.attributes[i].name + " " +elShape.attributes[i].value);
        ...
        case "g":
          ...
          if (attr.transform){
           ...
           // console.log("Match was: " + match);
            if (match.length == 3){
           //   console.log("Matched 3, whatever that means");
